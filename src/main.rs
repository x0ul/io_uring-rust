...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let socket_e = opcode::Socket::new(libc::PF_INET, libc::SOCK_STREAM, 0)
        .build()
        .user_data(1);
    unsafe {
        ring.submission()
    ...
    let sockaddr_in: *const libc::sockaddr_in = &libc::sockaddr_in {
        ...
        sin_addr: libc::in_addr {
            s_addr: u32::to_be(93 << 24 | 184 << 16 | 216 << 8 | 34),
        },
        sin_zero: [0; 8],
    };
    let sockaddr = unsafe {
        std::mem::transmute::<*const libc::sockaddr_in, *const libc::sockaddr>(sockaddr_in)
    };
    let addrlen = 16;
    let connect_e = opcode::Connect::new(types::Fd(sock_fd.0.as_raw_fd()), sockaddr, addrlen)
        .build()
        .user_data(2);
    unsafe {
        ring.submission()
    ...
    let http_get =
        "GET / HTTP/1.1\nHost: example.com\nUser-Agent: cody-rhea/42069\nAccept: */*\n\n"
            .as_bytes();
    let mut buf = vec![0; 1024 * 32]; // This is big enough for example.com,
                                      // but to do larger sites we'd want to
                                      // break up `recv` into multiple calls
